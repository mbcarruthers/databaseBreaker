...
DEFAULT_GOAL := help
.PHONY: databaseBreaker
project := databaseBreaker

databaseBreaker: build_server ## Keep this to display help when you just type 'make' in root dir

build_server: ## build the golang http server
	go build -o ${BIN_DIR}/$(project) ./cmd/
